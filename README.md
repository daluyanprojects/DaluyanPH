# Commit Message Convention

Use the following format for all commit messages:

```text
<type>: <short description>
```

## Types

| Type | Description |
|------|-------------|
| `feat` | Add a new feature |
| `fix` | Fix a bug |
| `docs` | Update documentation |
| `refactor` | Improve code without changing functionality |
| `test` | Add or update tests |
| `chore` | Perform maintenance tasks (e.g., dependencies, configuration) |

---

## Examples

```text
feat: add flood susceptibility prediction module
fix: resolve GeoTIFF upload validation error
docs: update setup guide
refactor: simplify raster preprocessing pipeline
test: add unit tests for prediction API
chore: update Docker configuration
```

---



# Thesis
libraries: 
npm init y
npm install express@4.18.2
npm install nodemand -D
npm install dotenv 
install mongoose@7.0.3
npm install cookie-parser
npm install uuid
npm install @upstash/ratelimit @upstash/redis

frontend: 
npm create vite@latest . 
npm install 
react-router
react-hot-toast
npm install -D tailwindcss@3 postcss autoprefixer
npx tailwindcss init -p
daisyui@4.12.24 -D
lucide-react
axios
react-router-dom


to run:
go to frontend folder then.
npm run dev

if may 'vite' is not recognized as an internal or external command,
operable program or batch file. just run "npm install"
